# 9.6 Global Digital Monetary Foundation -- Scraped Source Content

## Metadata
- **Purpose:** Full extracted text from each scraped link, keyed by link # from `links.md`
- **Status:** 1 DONE, 1 PARTIAL, 2 FAILED, 0 SKIPPED
- **Last updated:** April 11, 2026

---

## Extracted Content
### Link #1 -- "Rules not rulers" / Bitcoin as Manhattan Project of financial singularity (source of multiple quotes)

**Source:** www.youtube.com
**URL:** https://www.youtube.com/watch?v=MN4klUUx8fM
**Type:** YouTube
**Scrape status:** PARTIAL (YouTube (metadata only via oEmbed))

---

**Video Title:** Can Bitcoin Replace Government-Issued Money? A Debate
**Channel:** ReasonTV

*YouTube video -- transcript not available via automated scraping.*


---

### Link #2 -- @thewhyoffi: Bitcoin is civilization-building, base layer of money

**Source:** twitter.com
**URL:** https://twitter.com/thewhyoffi/status/1589680669785231360?s=46&t=pgFPZR3fAcd1-7IF2DFLOw
**Type:** Twitter/X
**Scrape status:** DONE

---

Something went wrong, but don’t fret — let’s give it another shot.

  


Try again

Some privacy related extensions may cause issues on x.com. Please disable them and try again.


---

### Link #3 -- Nic Carter: Bitcoin at 12 -- monetary manifesto, scalability via layers

**Source:** medium.com
**URL:** https://medium.com/@nic__carter/bitcoin-at-12-f6fce39cb9bb
**Type:** Article
**Scrape status:** FAILED (HTTP 403 (Cloudflare/JS-protected))

---

*No content extracted. Reason: HTTP 403 (Cloudflare/JS-protected)*


---

### Link #4 -- Nik Bhatia (@timevalueofbtc): Strategic Bitcoin Reserve as global settlement mechanism

**Source:** x.com
**URL:** https://x.com/timevalueofbtc/status/[embedded in source]
**Type:** Twitter/X
**Scrape status:** FAILED (HTTP 400)

---

*No content extracted. Reason: HTTP 400*

